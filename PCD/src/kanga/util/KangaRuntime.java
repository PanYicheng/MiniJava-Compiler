...
public class KangaRuntime {
    ...
    public KangaRuntime(Hashtable<Stmt, String> stmtInfo) {
        ...
        for (int i=0; i<registerName.length; i++) {
            registerHash[i]=new kanga.memory.Reg(registerName[i], null);
        }
    }
    ...
    public kanga.memory.Reg getReg(int i) {return registerHash[i];}
    public kanga.memory.Int getInt(String s) {
        if (integerHash.get(s)==null) integerHash.put(s, new Int(s));
        return integerHash.get(s);
    }
    public kanga.memory.Label getLabel(String s) throws KangaLabelException {
    	kanga.memory.Label label =  labelHash.get(s);
        if (label==null) {
    ...
    public HashMap<String, kanga.memory.Label> labelHash = new HashMap();
    public HashMap<String, int[]> procedureHash = new HashMap();

    // internal states
    protected java.util.Stack<kanga.memory.Stack> stackStack=new java.util.Stack();
    public Interp ivisitor=new Interp(this); // visitor
    public kanga.memory.Reg[] registerHash=new kanga.memory.Reg[registerName.length];
    ...
    public void initializeStack(kanga.memory.Stack s) {stackStack.push(s);}
    public kanga.memory.Stack getCurrentStack() {return stackStack.peek();}
    public kanga.memory.Stack pushStack(int paramSize,
                                  int stackSize,
                                  int argSize
        ...
        kanga.memory.Stack currentStack = stackStack.peek();
        kanga.memory.Stack newStack;
        try {
            newStack = new kanga.memory.Stack(paramSize, stackSize, argSize, 
                                        pc+1, false);
    ...
    public kanga.memory.Stack popStack() {
        if (stackStack.size()<=1) {
            System.err.println("Stack size is 1, cannot pop anymore"); 
            exception=true;
            return null;
        }            
        kanga.memory.Stack prevStack = stackStack.pop();
        kanga.memory.Stack currentStack = stackStack.peek();
        try {
            //System.out.println("--------------");
